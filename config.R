...
# One row per driver × k combination used in segmented regression.
...
# Defines where the baseline distribution is cut to form quantile-method
# thresholds.  Q_SPLIT_2S is the single split for k = 2 (default: median).
# Q_SPLIT_3S are the two split points for k = 3 (default: tertiles).
# These affect only the "quantile" candidate set, not STD_THRESHOLDS.

Q_SPLIT_2S <- 0.50          # Median split for k = 2
Q_SPLIT_3S <- c(1/3, 2/3)  # Tertile split for k = 3

# ---- Davies test significance level -----------------------------------------
# Alpha level for the Davies test for a non-zero breakpoint slope change.
# Used alongside MIN_DELTA_AIC; a candidate passes Stage 2 if either criterion
# is met (AIC gain OR significant Davies test).

DAVIES_ALPHA <- 0.05

# ---- Segmented regression controls -----------------------------------------
MIN_MONTHS_FOR_SEG <- 45    # Minimum data months required to attempt a fit
BOOT_B_SEG         <- 300   # Case-resampling bootstrap iterations for CI
MIN_DELTA_AIC      <- 2     # Minimum AIC gain of segmented over linear model
...
# Internal screening thresholds and sensitivity analysis parameters.
# These have been calibrated against the test data; change with caution.
# =============================================================================

# ---- Davies test granularity ------------------------------------------------
# Number of sub-intervals for the Davies test. Higher values give a more
# precise p-value at the cost of compute time. Record this value alongside
# outputs for reproducibility (results can differ across values of k).
DAVIES_K <- 10

# ---- Stage 3 hard-screen thresholds ----------------------------------------
S3_MIN_PCT_ASSIGNED   <- 0.90  # Min assignment completeness in ecological window
S3_MIN_SEASON_PROP    <- 0.10  # Min proportion for any single season level
S3_MEAN_MONTH_CONS    <- 0.55  # Min calendar-month consistency
S3_MIN_BLOCK_PROP     <- 0.05  # Min season proportion within any temporal block
S3_MIN_HEALTHY_BLOCKS <- 0.50  # Min fraction of blocks retaining all k levels
S3_BLOCK_YEARS        <- 2     # Block length (years) for temporal stability test

# ---- Stage 3 informational flags -------------------------------------------
# These never cause a candidate to be dropped; they are reported in the output
# so the analyst can judge whether flagged candidates are still appropriate.
S3_FLAG_KAPPA_LOW    <- 0.10  # κ below "slight" agreement (Landis & Koch 1977)
S3_FLAG_ALIGN_IQR    <- 1.5   # Threshold-breakpoint distance (IQR units)
S3_FLAG_OMEGA_SQ_LOW <- 0.01  # ω² below Cohen's "small" effect benchmark

# ---- Stage 4 near-constant flag ---------------------------------------------
# A Stage 2 ecological candidate is flagged as near-constant (label agreement
# metrics unreliable) if its dominant season class exceeds this proportion.
S4_NEAR_CONSTANT_THRESHOLD <- 0.95

# ---- Weight sensitivity grid (Stage 4) -------------------------------------
# The sensitivity analysis sweeps all (W_CLIMATE, W_ROBUST, W_VERIFY)
# combinations that sum to 1 and lie within these ranges.
